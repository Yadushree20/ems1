...
function Report() {
  ...
  const getProductionTimelineOptions = (productionData) => {
    ...
    // Set time range from 8 AM to 7 PM on the same day in UTC
    ...
    const endTime = baseDate.clone().set({ hour: 19, minute: 0, second: 0 });   // 7 PM UTC same day
    ...
    machineOrder.forEach((machineId, index) => {
      ...
      // If no data for this machine, show as OFF for the entire period
      if (timeline.length === 0) {
        ...
        if (timeline[0].startTime.valueOf() > startTime.valueOf()) {
          machinePoints.push({
            x: startTime.valueOf(),
            x2: Math.min(timeline[0].startTime.valueOf(), endTime.valueOf()),
            y: index,
            status: 'OFF',
            color: getStatusColor('OFF')
          });
      ...
      // Add all machine points to the main points array
    ...
    return {
      ...
      xAxis: {
        ...
        tickPositions: Array.from({ length: 12 }, (_, i) => {
          return startTime.valueOf() + (i * 3600 * 1000);
        }), // 12 hours from 8:00 to 19:00
        minPadding: 0,
        maxPadding: 0,
        startOnTick: true,
        endOnTick: true,
