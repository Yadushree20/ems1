...
function Report() {
  ...
  const getProductionTimelineOptions = (productionData) => {
    ...
    const statusCategories = ['PRODUCTION', 'ON', 'OFF'];
    const machineIds = [5, 4, 3, 2, 1, 7, 6]; // Your specific order of machine IDs

    // Create a map to store status timelines for each machine
    const statusTimelines = {};
    machineIds.forEach(machineId => {
      statusTimelines[machineId] = statusCategories.map((status, index) => ({
        x: startTime.valueOf(),
        x2: endTime.valueOf(),
        y: index, // Use index as y-coordinate for status
        status: status,
    ...
    // Process data points
    productionData.dataPoints.forEach(point => {
      const pointStart = moment(point.value[0]);
      const pointEnd = moment(point.value[1]);
      const statusIndex = statusCategories.indexOf(point.name);
      
      if (statusIndex !== -1) {
        statusTimelines[point.machine_id][statusIndex] = {
          x: Math.max(pointStart.valueOf(), startTime.valueOf()),
          x2: Math.min(pointEnd.valueOf(), endTime.valueOf()),
          y: statusIndex,
          status: point.name,
    ...
    const allPoints = Object.values(statusTimelines).flat();
    ...
    return {
      ...
      tooltip: {
        formatter: function() {
          const machine = machines.find(m => Number(m.id) === Number(this.point.machineId));
          const machineName = machine ? machine.machine_name : `Machine ${this.point.machineId}`;
