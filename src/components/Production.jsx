...
const Production = () => {
  ...
  const transformData = (rawData) => {
    ...
    // Filter out UNKNOWN and default_state data points
    const filteredData = {
      ...rawData,
      dataPoints: rawData.dataPoints.filter(
        point => !['UNKNOWN', 'default_state'].includes(point.name)
      )
    };
  ...
  const getChartOptions = (data, selectedMachine) => {
    ...
    const isToday = moment().isSame(startTime, 'day');
    
    // For historical dates, fill until 18:00, for today fill until current time
    const endOfDay = moment.utc(startTime).add(10, 'hours').valueOf(); // 18:00
    const maxTime = isToday ? Math.min(endTime.valueOf(), currentTime) : endOfDay;
    ...
    return {
      ...
      xAxis: {
        ...
        labels: {
          format: '{value:%H:%M}',
          formatter: function() {
            return `<span style='font-weight: bold'>${moment.utc(this.value).format('HH:mm')}</span>`;
          },
          useHTML: true,
          style: { 
            fontSize: '14px',
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 500) {
          ...
          setHasNoData(true);
          message.info('No production data available for the selected machine and date');
          return;
        }
          ...
          >
            {hasNoData ? (
              <div style={{
                ...
                No data available for the selected machine and date
              </div>
            ) : productionData ? (
