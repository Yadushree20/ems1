...
function Report() {
  ...
  const getProductionTimelineOptions = (productionData) => {
    ...
    if (!productionData?.dataPoints) {
      console.error('No dataPoints property in production data');
      return {};
    }
    
    if (productionData.dataPoints.length === 0) {
      console.log('Data points array is empty for the selected date');
      return {};
    }

    // Log the first few data points for debugging
    console.log('First 3 data points:', productionData.dataPoints.slice(0, 3).map((point, idx) => ({
      index: idx,
      machine_id: point.machine_id,
      status: point.name,
      timestamps: point.value,
      startTime: point.value?.[0] ? moment.utc(point.value[0]).format('YYYY-MM-DD HH:mm:ss [UTC]') : 'invalid',
      endTime: point.value?.[1] ? moment.utc(point.value[1]).format('YYYY-MM-DD HH:mm:ss [UTC]') : 'invalid'
    })));
    ...
    if (validPoints === 0) {
      console.error('No valid points found in the selected date range. Possible issues:', {
        dateRange: {
        ...
        firstFewTimestamps: productionData.dataPoints.slice(0, 3).map(p => ({
          start: p.value?.[0],
          end: p.value?.[1],
