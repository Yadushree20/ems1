...
// import { fetchAllMachineStates } from './machineState';
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch all machine states using the new function
        const allMachineStatesData = await fetchAllMachineStates();

        console.log('Machines Data:', machinesData);
        console.log('All Machine States Data:', allMachineStatesData);

        // Create machine data with default status
        const machinesWithStatus = machinesData.map(machine => {
          return {
            ...machine,
            status: 'OFF' // Default to 'OFF' for all machines 
          };
        });

        // Update status for each machine from all machine states data
        if (allMachineStatesData && Array.isArray(allMachineStatesData)) {
          allMachineStatesData.forEach(stateData => {
            const machineToUpdate = machinesWithStatus.find(
              machine => machine.id === stateData.machine_id || 
                         machine.id === parseInt(stateData.machine_id)
            );
            
            if (machineToUpdate) {
              // Update the machine status based on the state data
              // Convert state to uppercase to match our status constants
              machineToUpdate.status = stateData.state?.toUpperCase() || 'OFF';
            }
          });
        }
        ...
        // Attempt to show machines even if status fetch fails
        try {
          const machinesResponse = await fetch(getMachinesURL());
          const machinesData = await machinesResponse.json();
