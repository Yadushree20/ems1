...
import { Layout, Typography, Spin, Table, message, Button, Space } from 'antd';
...
import { PrinterOutlined, ArrowLeftOutlined, FilePdfOutlined } from '@ant-design/icons';
...
function Report() {
  ...
  const getProductionTimelineOptions = (productionData) => {
    ...
    // Define all machine names in the correct order (matching your requirement)
    const machineCategories = [
      'LT 500',
      'Mono200',
      'MCV450',
      'Mazak H-400',      // Adding machines below LT 500
      'Schaublin33 CNC',
      'HMT VTC 800',
      'HMT Stallion 200'
    ];

    // Create empty data points for all machines
    let allPoints = [];

    // Add default 'OFF' status for all machines
    machineCategories.forEach((machineName, index) => {
      allPoints.push({
        x: startTime.valueOf(),
        x2: endTime.valueOf(),
        y: index,
    ...
    const dataPoints = productionData.dataPoints.map(point => {
      const machineName = getMachineName(point.machine_id);
      const machineIndex = machineCategories.indexOf(machineName);
      if (machineIndex !== -1) {
        return {
          x: Math.max(moment(point.value[0]).valueOf(), startTime.valueOf()),
          x2: Math.min(moment(point.value[1]).valueOf(), endTime.valueOf()),
          y: machineIndex,
          name: point.name,
    ...
    // Combine all points
    allPoints = [...allPoints, ...dataPoints];

    return {
      chart: {
        type: 'xrange',
        height: 500,  // Increased height to accommodate all machines
        marginLeft: 150,  // Increased margin for machine names
      ...
      yAxis: {
        ...
        labels: {
          ...
          padding: 4,
          x: -10  // Adjust label position
        },
        gridLineWidth: 1,
        min: 0,
        max: machineCategories.length - 1  // Ensure all categories are shown
      },
      tooltip: {
        formatter: function() {
          ...
          return `<b>${this.point.machineName}</b><br/>
                  Status: ${this.point.name}<br/>
      ...
      plotOptions: {
        xrange: {
          borderRadius: 0,
          pointWidth: 30,  // Increased point width
          colorByPoint: false
        }
      },
      series: [{
  ...
  const handleBack = () => {
    navigate('/energymonitoring/productivity');
  };
  ...
  const handleExportPDF = async () => {
    try {
      ...
      // Add title
      pdf.setFontSize(16);
      pdf.text('Workshop Production Report', 15, 15);

      // Add date
      pdf.setFontSize(12);
      pdf.text(`Date: ${moment().format('MMMM D, YYYY')}`, 15, 25);
      ...
      // Save the PDF
      pdf.save('workshop-production-report.pdf');
    } catch (error) {
      console.error('Error generating PDF:', error);
      message.error('Failed to generate PDF');
    }
