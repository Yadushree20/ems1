...
function Report() {
  ...
  const getProductionTimelineOptions = (productionData) => {
    // Check if productionData has dataPoints and it's an array
    const dataPoints = productionData?.dataPoints;

    if (!Array.isArray(dataPoints) || dataPoints.length === 0) {
      console.log('No data points available or productionData.dataPoints is not an array'); // Debug log
      return {};
    }
    ...
    // Add actual data points from the response
    const dataPointsMapped = dataPoints.map(point => {
      const machineIndex = machineCategories.indexOf(point.machineName);
      
      if (machineIndex !== -1) {
        return {
          x: point.x, // Start time in epoch
          x2: point.x2, // End time in epoch
          y: machineIndex,
          name: point.name,
          color: point.color, // Use the color from the response
    ...
    // Combine all points
    allPoints = [...allPoints, ...dataPointsMapped];
    ...
    return {
      ...
      yAxis: {
        ...
        gridLineWidth: 1,
        min: 0,
        max: machineCategories.length - 1
      },
      tooltip: {
        formatter: function() {
