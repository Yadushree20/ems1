...
function Report() {
  ...
  const getProductionTimelineOptions = (productionData) => {
    ...
    // Set time range from 8 AM selected day to 8 AM next day
    const baseDate = moment(date).startOf('day');
    const startTime = baseDate.clone().set({ hour: 8, minute: 0, second: 0 });  // 8 AM selected day
    const endTime = baseDate.clone().add(1, 'day').set({ hour: 8, minute: 0, second: 0 });   // 8 AM next day
    
    console.log('Date range for production data:', {
      selectedDate: date,
    ...
    const sortedPoints = productionData.dataPoints
      .filter(point => {
        ...
        // Include points that overlap with our time range
        return (
          (pointStart.valueOf() >= startTime.valueOf() && pointStart.valueOf() <= endTime.valueOf()) ||
          (pointEnd.valueOf() >= startTime.valueOf() && pointEnd.valueOf() <= endTime.valueOf()) ||
          (pointStart.valueOf() <= startTime.valueOf() && pointEnd.valueOf() >= endTime.valueOf())
        );
      })
      .sort((a, b) => moment(a.value[0]).valueOf() - moment(b.value[0]).valueOf());
      
    console.log('Filtered production points:', sortedPoints);
